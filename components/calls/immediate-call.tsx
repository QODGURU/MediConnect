...
export function ImmediateCall() {
  ...
  const [scriptTemplate, setScriptTemplate] = useState("appointment")
  ...
  const handleScriptTemplateChange = (value: string) => {
    setScriptTemplate(value)
    switch (value) {
      case "appointment":
        setScript(DEFAULT_APPOINTMENT_REMINDER)
        break
      case "followup":
        setScript(FOLLOW_UP_CALL)
        break
      case "testresults":
        setScript(TEST_RESULTS_CALL)
        break
      case "medication":
        setScript(MEDICATION_REMINDER)
        break
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/calls/immediate", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          appointmentReason: "your check-up",
          condition: "recent health concerns",
          lastAppointmentDate: "last week",
          testType: "blood work",
          testDate: "last Tuesday",
          medicationName: "your prescription medication",
        }),
      })
      ...
      notifications.success(`Call initiated successfully! Call ID: ${data.callId}`)
  ...
  return (
    <Card className="bg-white border-blue-200 shadow-lg">
      <CardHeader className="bg-blue-700 text-white rounded-t-lg">
        <CardTitle className="text-xl font-bold">Make Immediate Call</CardTitle>
        <CardDescription className="text-blue-100">Call a patient immediately using Retell AI</CardDescription>
      </CardHeader>
      <CardContent className="p-6">
        <Alert className="mb-4 bg-blue-50 border-blue-200">
          ...
          <AlertDescription className="text-blue-700">
            ...
            <p className="mt-1">
              The script uses variables in the format {"{variable_name}"}, such as {"{patient_name}"} and{" "}
              {"{doctor_name}"}. These will be replaced with the values you provide.
            </p>
          </AlertDescription>
        </Alert>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="scriptTemplate" className="text-blue-800 font-medium">
              Script Template
            </Label>
            <Select value={scriptTemplate} onValueChange={handleScriptTemplateChange}>
              <SelectTrigger className="border-blue-200 focus:border-blue-500 focus:ring-blue-500 transition-all">
                <SelectValue placeholder="Select a script template" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="appointment">Appointment Reminder</SelectItem>
                <SelectItem value="followup">Follow-up Call</SelectItem>
                <SelectItem value="testresults">Test Results</SelectItem>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-gray-500">
              Edit the script as needed. Variables like {"{patient_name}"} will be replaced with the values you provide.
            </p>
          </div>
        </form>
      </CardContent>
        ...
        >
          {loading || isPolling ? (
            <>
              ...
              {loading ? "Calling..." : "Monitoring Call..."}
            </>
          ) : (
            <>
